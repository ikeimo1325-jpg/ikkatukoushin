...
const SITE_NAME = "nightstyle" as const;

// TODO: ナイトスタイルのログインURLを設定
// const LOGIN_URL = "https://...";

export class NightstyleUpdater implements PortalUpdater {
  siteName = SITE_NAME;
  ...
  async updateAttendance(payload: AttendanceUpdatePayload): Promise<UpdateResult> {
    ...
    if (dryRun) {
      ...
      return {
        ...
        message: `[DryRun] ナイトスタイル: ${castName} ${date} ${status} の更新をシミュレートしました`,
      };
    }

    // TODO: ナイトスタイルの自動操作を実装する
    return {
      site: SITE_NAME, castId, castName, success: false,
      message: "ナイトスタイルの自動操作は未実装です",
    };
  }
}
